...
The Scala ecosystem rich as it is thanks to this wide range of applications
...
- optimization macros like those in spire/parboiled are widely used in main sources.
- code generation macros like for serialization are also widely used, given that
  the best practice is  to invoke them only once per type signature.
...
Please get in touch with me if you would like to add your favorite projects (esp.
application sources!) to the corpus or learn how to run the analysis on a closed
source project.
It's quite easy.
...
Another important attribute of Squid quasiquotes is that they are reusable.
This means it's possible to write generic utility functions with Squid
quasiquotes, which is a common limitation of alternative statically typed
quasiquote systems.
...
using question marks `code"(?x: Int) + 1)` and free variables are then tracked in
the second parameter of quasiquote's type `Code[Int, {x: Int}]`.
...
There are still many open question on how to incorporate Squid with the new macros.
For example,
...
Our next steps is to continue the work in the [scalacenter/macros] repository,
